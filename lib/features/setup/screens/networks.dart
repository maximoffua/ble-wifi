                  ...
                  child: accessPoints.isEmpty
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, i) => _AccessPointTile(
                                ...
                                onTap: (creds) async {
                                  final service = widget.device.servicesList
                                      .firstWhere((element) =>
                                          element.uuid == miniGroServiceId);
                                  final ssid = service.characteristics
                                      .firstWhere(
                                          (element) => element.uuid == ssidChr);
                                  final pass = service.characteristics
                                      .firstWhere(
                                          (element) => element.uuid == passChr);
                                  talker.info(
                                      "Connecting to ${creds.ssid}\n  using password: ${creds.password}");
                                  await ssid.write(creds.ssid.codeUnits);
                                  await pass.write(creds.password.codeUnits);
                                  talker.info(
                                      "SSID ${String.fromCharCodes(await ssid.read())}");
                                },
                              )),
                ),
              ),
